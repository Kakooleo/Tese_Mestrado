...
\noindent $f_0$ is the initial frequency, $\alpha=B/T$ is the frequency sweep, $f_c = f_0 + B/2$ is the center of the bandwith, $N_1(t)$ and $N_2(t)$ are additive noise terms, and $T$ is the duration of the signals, such that $BT \gg 1$. The time representation and frequency representation of the $x_1$ and $x_2$ signal are shown in Figures \ref{fig:lim-tf-rep-a} and \ref{fig:lim-tf-rep-b}, respectively. As it can be seen, both have a similar frequency representations, but their time representations are very different. More than that, taking a look at the time representations, it is hard to identify which frequencies dominate at which time, or even if there is more than one frequency present. It is also difficult to identify the evolution of the frequencies over time, a characteristic of nonstationary signals. This is a limitation of the classical representations of the signal. In the Sections (that follow) [\textcolor{red}{colocar aqui as secoes depois}], a more complex time-frequency representation will be presented.
