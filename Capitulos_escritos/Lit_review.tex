...
\section{Basics of Signal Analysis} \label{sec-signal-analysis}

In this section the basics of signal analysis will be presented. The main idea behind this section is to provide the theoretical background that will be used in the next chapters, specially when dealing with mass and velocity effects on acceleration data. 

\textcolor{red}{(No futuro escrever melhor essa parte aqui, talvez seja interessante adicionar ou remover algumas sessoes se essas nao forem utilizadas pela tese.)}

\subsection{Types of Signals} \label{sec-types-signal}

In Section \ref{sec-time-series} a time sereis was defined as a sequence of data points indexed sequentially over time. A time series can be considered a signal, which is a function that conveys information about the behavior of a system, such as an acoustic wave from a speech, an eletronic wave carrying information of an image or, in our case, the output of an acceleromter sensor \cite{priemer1991introductory}. Signals can be classified into six different types depending on the level of complexity \cite{Boashash2016ChapterITimeFrequency}:
\begin{enumerate}
    \item \textbf{Type 1 Signal}: This type of signal represents a simple and deterministic sinusoidal wave, described by a constant frequency $f_c$, a constant amplitude and phase. This type of signal is shown in Figure \ref{fig:type_signal-a}.
    \item \textbf{Type 2 Signal}: This type of signal is a multicomponent signal, which is a liner combination of several type 1 sinusoidal waves, each with a different frequency, amplitude and phase. This type of signal is illustrated in Figure \ref{fig:type_signal-b}.
    \item \textbf{Type 3 Signal}: This type of signal is comprised of a single component with constant amplitude and phase, but with a time-varying frequency $f_i(t)$. This type of signal is shown in Figure \ref{fig:type_signal-c}. One example of this is the frequency modulated (FM) signal used in telecommunication systems. The dominant frequency of this signal at time $t$ is called the instantaneous frequency (IF) and will be discussed in more detail in Section \ref{sec-IF}.
    \item \textbf{Type 4 Signal}: This type of signal is a multicompening signal composed of multiple type 3 signals summed together, each with a different time-varying frequency, amplitude and phase. This type of signal is illustrated in Figure \ref{fig:type_signal-d}.
    \item \textbf{Type 5 Signal}: A type 5 signal is a type 4 signal with an white noise added to the sum of the components and a white noise added to the amplitude of each component. This type of signal is shown in Figure \ref{fig:type_signal-e}. The first type of noise is called an additive noise, while the second is called a multiplicative noise. 
    \item \textbf{Type 6 Signal}: The type 6 signal is a sinc function with additive withe noise added to it. Figure \ref{fig:type_signal-f} shows an example of this type of signal. This type of signal is the most important for this dissertation, as it reveals possible defects on the railway line.
\end{enumerate}

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=7cm]{Cap2_LitReview/Signals/Types_of_signals/type_1_signal.png}
        \caption{}
        \label{fig:type_signal-a}
    \end{subfigure}
    \hspace{0.2cm}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=7cm]{Cap2_LitReview/Signals/Types_of_signals/type_2_signal.png}
        \caption{}
        \label{fig:type_signal-b}
    \end{subfigure}
    \\
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=7cm]{Cap2_LitReview/Signals/Types_of_signals/type_3_signal.png}
        \caption{}
        \label{fig:type_signal-c}
    \end{subfigure}
    \hspace{0.2cm}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=7cm]{Cap2_LitReview/Signals/Types_of_signals/type_4_signal.png}
        \caption{}
        \label{fig:type_signal-d}
    \end{subfigure}
    \\
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=7cm]{Cap2_LitReview/Signals/Types_of_signals/type_5_signal.png}
        \caption{}
        \label{fig:type_signal-e}
    \end{subfigure}
    \hspace{0.2cm}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=7cm]{Cap2_LitReview/Signals/Types_of_signals/type_6_signal.png}
        \caption{}
        \label{fig:type_signal-f}
    \end{subfigure}
    \caption{The six types of signals. \textbf{(a)} Type 1 signal represented by a single sinusoidal wave with constant frequency, amplitude and phase. \textbf{(b)} Type 2 signal represented by a linear combination of several type 1 signals. \textbf{(c)} Type 3 signal represented by a single component with constant amplitude and phase, but with a time-varying frequency. \textbf{(d)} Type 4 signal represented by a linear combination of several type 3 signals. \textbf{(e)} Type 5 signal represented by a type 4 signal with additive and multiplicative white noise. \textbf{(f)} Type 6 signal represented by a sinc function with additive white noise. \cite{Boashash2016ChapterITimeFrequency}.}
    \label{fig:type_signal}
\end{figure}

It is important to highlight that an algorithm that extracts characteristics from a signal changes with the type of signal, meaning that an algorithm that works well for a type 1 signal might not work well for a type 4 signal \cite{Boashash2016ChapterITimeFrequency}.

\subsection{Fourier Transform}\label{sec-FT}

In Section \ref{sec-types-signal}, the six different types of signals and their representation in the time domain were presented. However, signals can also be represented in the frequency domain. The Fourier Transform (FT) is a mathematical tool that maps a time-domain signal to its frequency-domain representation by decomposing it into its constituent frequency components. Formally, the FT is a linear operator defined on suitable function spaces. For example, if $f(t) \in L^1(\mathbb{R})$, where $L^1(\mathbb{R})$ denotes the Banach space of absolutely integrable functions, the Fourier Transform is defined as \cite{grochenig2001foundations}
...
\noindent where $\omega$ denotes the frequency variable. Since the Fourier Transform is invertible on appropriate function spaces, the original time-domain signal can be recovered from its frequency-domain representation using the inverse Fourier Transform (IFT), defined as
...
\noindent where $F(\omega)=\mathcal{F}[f](\omega)$ is the Fourier Transform of $f(t)$. Notice that to get back the signal to the time domain, the IFT integrates over all frequencies, including negative ones. The mathematical explanation for this is that if $f(t)$ is a real-valued signal, then its FT satisfies the property $F(\omega) = {F^*(-\omega)}$, where ${F^*(\omega)}$ denotes the complex conjugate of $F(\omega)$, i.e., its FT has a Hermitian symmetry. This means that the negative frequencies are duplicates from the positive ones \cite{Boashash2016ChapterITimeFrequency}.

Another important result to discuss is the Parseval's theorem. It states that if $f \in L^1 \cap L^2(\mathbb{R})$, then:

\begin{equation}
    E_s = \int_{-\infty}^{\infty} |f(t)|^2\,dt = \int_{-\infty}^{\infty} |\mathcal{F}[f](\omega)|^2\,d\omega
    \label{eq:Parseval}
\end{equation}

\noindent where $E_s$ is the energy of the signal \cite{grochenig2001foundations}. This means that the energy of the signal is preserved by the FT, which allows for the identification of frequencies that contribute the most to the signal's energy. 


\subsection{Power Spectral Density} \label{sec-PSD}

In Section \ref{sec-FT}, it is shown that taking the FT of a signal preserves the total energy of the signal, which allows for the identification of the frequencies that contribute the most to the signal's energy. However, this only applies to absolutely integrable functions, that is, functions whose total energy $E$ is finite. This does not hold, however, for a general stationary stochastic signal, whose energy is usually infinite \cite{kay1981spectrum}. In this case, it is more useful to analyze the distribution of power (energy per unit of time) across the frequency spectrum, which is captured by the Power Spectral Density (PSD).

Formally, for a signal $x(t)$, the PSD is defined as \cite{youngworth2005overview}
\begin{equation}
    S_x(\omega) = \lim_{T \to \infty} \frac{1}{T} \, \left| X_T(\omega) \right|^2,
    \label{eq:psd_definition}
\end{equation}
where $X_T(\omega)$ denotes the Fourier Transform of the signal truncated to an observation window of length $T$. This truncation is necessary so that the definition remains valid for signals that do not have a formal Fourier Transform.

An important result is the Wiener--Khinchin theorem, which relates the PSD to the signal's autocorrelation function $R_x(\tau)$ \cite{kay1981spectrum},
\begin{equation}
    S_x(\omega) = \int_{-\infty}^{\infty} R_x(\tau) \, e^{-i 2\pi \omega \tau} \, d\tau,
    \label{eq:wiener_khinchin}
\end{equation}
\noindent where $R_x(\tau)$ is defined as:
\begin{equation}
    R_x(\tau) = \mathbb{E}[x(t + \tau)x^*(t)].
\end{equation}

\subsection{Limitations of Time and Frequency Representations} \label{sec-lim-tf-r}

As discussed in Section \ref{sec-FT}, the FT can convert a signal from the time domain to the frequency domain. However, relying only on these two representations can be limiting, as the variables $t$ and $\omega$ are treated independently, i.e., to obtain one, the other is integrated out, which causes the representations to be nonlocalized w.r.t. the excluded variable \cite{Boashash2016Chapter1TimeFrequency}. For example, taking the FT of a signal $f(t)$, the resulting frequency-domain representation $F(\omega)$ provides information about the frequency content of the signal but does not provide any information about when those frequencies occur in time. This limitation is particularly problematic for non-stationary signals, where the frequency content changes over time. This is clearly shown by Boashash \cite{Boashash2016ChapterITimeFrequency} using the following two signals:

\begin{equation}
    x_{1}(t) = \text{rect}\left(\frac{t-T/2}{T}\right)\cos \left[2\pi \left(f_0t + \alpha \frac{t^2}{2}\right)\right] + N_1(t)
    \label{eq-xA}
\end{equation}
\begin{equation}
    x_{2}(t) = \text{rect}\left(\frac{t-T/2}{T}\right)\frac{\sin\left(\pi B t\right)}{\pi B t} \cos \left(2\pi f_c t\right) + N_2(t)
    \label{eq-xB}
\end{equation}

\noindent where $\text{rect}(\cdot)$ is the function
\begin{equation}
...
\noindent $f_0$ is the initial frequency, $\alpha=B/T$ is the frequency sweep, $f_c = f_0 + B/2$ is the center of the bandwidth, $N_1(t)$ and $N_2(t)$ are additive noise terms, and $T$ is the duration of the signals, such that $BT \gg 1$. The time and frequency representations of the $x_1$ and $x_2$ signal are shown in Figures \ref{fig:lim-tf-rep-a} and \ref{fig:lim-tf-rep-b}, respectively. As it can be seen, both have similar frequency representations, but their time representations are very different. Futhermore, taking a look at the time representations, it is hard to identify which frequencies dominate at which time, or even if there is more than one frequency present. It is also difficult to identify the evolution of the frequencies over time, a characteristic of nonstationary signals. This is a limitation of the classical representations of the signals. In the Section \ref{sec-tfd}, a more complex time-frequency representation will be presented.

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.98\textwidth}
        \includegraphics[width=0.98\textwidth]{Cap2_LitReview/Signals/Lim_tf_rep/LFM_signal.png}
        \caption{}
        \label{fig:lim-tf-rep-a}
    \end{subfigure}
    \\
    \begin{subfigure}{0.98\textwidth}
        \includegraphics[width=0.98\textwidth]{Cap2_LitReview/Signals/Lim_tf_rep/chirp_signal.png}
        \caption{}
        \label{fig:lim-tf-rep-b}
    \end{subfigure}
    \caption{Time and frequency representations of the signals defined in Equations \ref{eq-xA} and \ref{eq-xB}. \textbf{(a)} Time and frequency representations of the signal $x_1$. \textbf{(b)} Time and frequency representations of the signal $x_2$. \cite{Boashash2016ChapterITimeFrequency}}
\end{figure}

\subsection{Analytic Signals and the Hilbert Transform}\label{sec-analytic-signals}

In Section \ref{sec-FT} it is discussed about negative frequencies and the Hermitian symmetry of real-valued signals. More than that, a signal $f(t)$ is said to be real if and only if:
\begin{equation}
    F(\omega) = {F^*(-\omega)}
\end{equation}
\noindent where $F(\omega)$ is the FT of $f(t)$. In this case, the negative part of the frequencies can be discarded withouth the loss of information. In fact, it allows the signal to be sampled at half the Nyquist rate \cite{WignerDistBoashash}. This duplication of information allows the negative frequencies to be discarded without losing of information, which forms an analytic signal. An analytic signal $z(t)$ is a complex-valued signal that contains only positive frequency components, that is:
\begin{equation}
    Z(\omega) = 0, \quad \text{for } \omega < 0
\end{equation}

Using this definition, it is possible to state the following theorem \cite{Boashash2016Chapter1TimeFrequency}:
\begin{theorem}
    Consider two real signals $s(t)$ and $y(t)$. The signal
    \begin{equation}
        z(t) = s(t) + iy(t)
        \label{eq:analytic-conjugate}
    \end{equation}
    is analytic with real DC component if and only if
    \begin{equation}
        Y(\omega) = -i \text{sgn}(\omega) S(\omega)
        \label{eq:FT-relations}
    \end{equation}
    where $S(\omega)$ and $Y(\omega)$ are the Fourier transforms of $s(t)$ and $y(t)$, respectively, and 
    \begin{equation}
...
The proof of this theorem is presented in Appendix \ref{appendix-b}.

If $y(t)$ nd $s(t)$ are related by Equation \ref{eq:FT-relations}, then $y(t)$ is called the Hilbert transform (HT) of $s(t)$ and denoted as:
\begin{equation}
    y(t) = \mathcal{H}\{s(t)\}.
    \label{eq:HT}
\end{equation}
\noindent Using the HT, it is possible to construct the analytic signal $z(t)$:
\begin{equation}
    z(t) = s(t) + i\mathcal{H}\{s(t)\}.
\end{equation}
\noindent In this case,$z(t)$ is called the analytic associate of $s(t)$. 

By taking the IFT of Equation \ref{eq:FT-relations}, combining it with Equation \ref{eq:HT} and using the convolution property of the FT, it is possible to show that the HT can be defined as:
\begin{equation}
    \mathcal{H}\{s(t)\} = \frac{1}{\pi t} * s(t) 
    = \frac{1}{\pi} \text{p.v.} \int_{-\infty}^{\infty} \frac{s(\tau)}{t-\tau} d\tau
    \label{eq:HT-definition}
\end{equation} 

The reasons for using $z(t)$ and the HT instead of $s(t)$ will be highlighted in Section \ref{sec-tfd}.

\subsection{Instantaneous Frequency}\label{sec-IF}

As stated in Section \ref{sec-analytic-signals}, given a real-valued signal $s(t)$, it is possible to construct an analytical signal $z(t)=s(t)+i\mathcal{H}\{s(t)\}$, where $\mathcal{H}\{s(t)\}$ is the Hilbert transform of $s(t)$. This analytic signal can be expressed as:
\begin{equation}
    z(t) = a(t) e^{i\phi(t)},
\end{equation}
\noindent where $a(t)$ is the amplitude envelope and $\phi(t)$ is the phase of the signal. The instantaneous frequency (IF) of $s(t)$ is then defined as the rate of change of this phase, $f_i(t) = \frac{1}{2\pi}\frac{d\phi}{dt}$, extending the notion of frequency to signals whose spectral content may vary over time. 

As an example, consider the case of a pure cosine, $x(t) = A\cos(2\pi f_0 t)$, whose analytic signal has phase
\begin{equation}
    \phi(t) = 2\pi f_0 t.
    \label{eq:IF-def}
\end{equation}
The phase here increases linearly with time, at a constant rate of $2\pi f_0$ radians per second. Since a phase advance of $2\pi f_0$ radians per second is equivalent to $f_0$ complete cycles per second, the rate of change of phase coincides with the frequency of the signal in this elementary case. This observation does not depend on the phase being linear in time \cite{Stankovi2014}. When $\phi(t)$ advances more quickly during some intervals and more slowly during others, the signal oscillates faster or slower accordingly, even though a single frequency no longer characterizes the waveform as a whole.
 
The definition above, however, is subject to controversy. For example, Shekel \cite{shekel1953instantaneous} argues that IF defined in Equation \ref{eq:IF-def} is not a unique function of time because, for example, a amplitude modulated (AM) signal can be defined as $a(t)e^{i2\pi f_0 t}$, with $a(t)$ being a complicated function, or as $a_0 e^{i\phi(t)}$, with $a_0$ being a constant and $\phi(t)$ being a complicated phase function. Altough the method of creating the analytic signal described in Section \ref{sec-analytic-signals} is a unique way to define the phase of a signal, the physical meaning of the HT, $a(t)$ and $\phi(t)$ is not always meaningful and representative. Boashash \cite{boashash1992estimating} gives the conditions for this transformation to be representative of the physical phenomena, but this discussion is beyond the scope of this dissertation.

This restriction becomes more apparent when the definition is applied to a multicomponent signal, formed by the superposition of two or more oscillatory components with distinct frequencies present simultaneously. In this case, each component of the signal will have its own instantaneous frequency and, in general, the IF of the sum of two signals is not the sum of the IFs of the individual components \cite{Boashash2016Chapter1TimeFrequency}. In fact, $f_i(t)$, as defined above, behaves as a kind of instantaneous average, a weighted combination of the frequencies of the individual components, weighted by their relative contributions to the signal's energy at that instant \cite{cohen1988instantaneous,boashash1992estimating}. The resulting value may fall between the true component frequencies, or even outside their range altogether, and therefore cannot be interpreted as the frequency of any physical oscillation.

This limitation motivates a reinterpretation of instantaneous frequency in terms of energy distribution over time and frequency. Rather than describing the frequency of the signal at time $t$, $f_i(t)$ is more appropriately understood as an estimate of the frequency around which most of the signal's energy is concentrated at that instant, when the signal is examined through a joint time-frequency distribution \cite{stankovic2014instantaneous}. This concept will be discussed further in Section \ref{sec-tfd}, where time-frequency distributions will be presented.


\section{Time Frequency Distributions} \label{sec-tfd}

In this section, some of the most important time-frequency distributions (TFDs) are presented. The core idea behind TFDs is to provide a joint representation of a signal such that the area under the TFD surface corresponds to the energy of the signal, that is, the TFD is an energy density function \cite{Boashash2016Chapter2HeuristicFormulation}. For a monocomponent signal, the ideal TFD would concentrate all the energy of the signal along its IF \cite{stankovic1995analysis}. Mathematically, this can be expressed as:
\begin{equation}
    ITF(t, \Omega) = 2\pi A^q(t) \delta(\Omega - \phi'(t)) = A^q(t) \int_{-\infty}^{\infty} e^{i\phi'(t) \tau} e^{-i\Omega \tau} d\tau.
    \label{eq:ITF}
\end{equation}
However, as will be shown in the next sections, in practice, the TFDs only approximate this ideal behavior, and the energy of the signal is spread around the IF.

In Sections \ref{sec-STFT} and \ref{sec-WT}, the so-called linear TFDs will be presented. These TFDs try to represent the signal as a linear combination of elementary signals, called atoms, which are localized in time and frequency. In Section \ref{sec-WVD}, the quadratic TFDs will be presented. Quadratic TFDs distribute the energy of the signal in the time-frequency plane. As will be shown, these TFDs are all based on the Wigner-Ville distribution (WVD).

Finally, another important point to highlight is that in the next sections the TFDs will be presented using the analytic conjugate of the signal, as using the real signal can lead to some problems. Check Appendix \ref{appendix-c} for more details.

\subsection{Short-Time Fourier Transform} \label{sec-STFT}

Given a signal $s(t)$ and a real, even window function $w(t)$, the Short-Time Fourier Transform (STFT) is defined as \cite{grochenig2001foundations}:
\begin{equation}
    F^{w}_{s}(t, \omega) = \int_{-\infty}^{\infty} s(\tau) w(\tau - t) e^{-i2\pi \tau \omega} d\tau,
\end{equation}
The STFT 

\subsection{Wavelet Transform} \label{sec-WT}


\subsection{Quadratic Time-Frequency Distributions} \label{sec-WVD}
...
Railway track irregularities are normally modeled as being random and, for this reason, are typically described by a PSD function, see Section \ref{sec-PSD}. Countries like USA, China, Britain and Germany adopted this approach to describe track irregularities \cite{PIRES2021107806,Berawi2013ImprovingRT}. The PSD function parameters were estimated from measured data and, therefore, represent the average irregularity observed in each respective railway network.
...
\section{How to Generate Track Irregularities from PSDs} \label{sec-track-irr-PSD}

As stated in Section \ref{sec-FRA-Standard}, track irregularities are modeled as random processes described by a PSD function. The PSDs, however, does not describe the track irregularity profile directly, but rather the distribution of the energy of the signal over the spatial frequency $\Omega$. To obtain the track irregularity, it is used an approach called the spectral representation method \cite{HU1997105}, for more details see Appendix \ref{appendix-d}. The idea behind this method is to split the PSD into N grids that are spaced by $\Delta \Omega$ and, each grid is then represented by a cosine wave with frequency $\Omega_i$ and the track irregularity profile is then obtained by the sum:
\begin{equation}
    r(\chi) = \sum_{i=1}^{N} A_i \cos(\Omega_i \chi + \phi_i)
    \label{eq:track_irregularity}
\end{equation}
where $\chi$ is the spatial coordinate, $A_i = \sqrt{2 S(\Omega_i) \Delta \Omega}$ is the amplitude of the i-th cosine wave, $\Omega_i$ is the spatial frequency, $\phi_i ~ \mathcal{U}(0, 2\pi)$ is a pseudo-random phase, and $S(\Omega_i)$ is the PSD function evaluated at $\Omega_i$. 

\section{Assessing Railway Track Condition} \label{sec-assessing-railway-track}

Maintaining railway tracks in good condition is crucial to ensure safe and comfortable operations of trains \cite{Tsunashima-2019}. This maintenance is achieved through continuous monitoring of track conditions and the detection of irregularities. There are now two main methods to assess track quality: using track geometry cars (TGC) to directly measure the physical geometry of the track, and monitoring the dynamic response of the train using instrumented railway vehicles (IRV) equipped with onboard sensors capable of measuring the dynamics of the system \cite{PIRES2024107191}.

TGCs are equipped with sophisticated systems that directly measure the geometry of the track, that can be later compared to regulatory standards limits to identify parts of the track that need maintenance. However, this method has several drawbacks \cite{GHIASI2025109516}:
\begin{itemize}
...
Using IRVs instead of the TGCs has several advantages, such as \cite{Hironori_ONO202322-00239}:
\begin{itemize}
...
Despite that, it is important to highlight that data collection using IRVs also comes with some drawbacks, such as \cite{GHIASI2025109516}:
\begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=14cm]{Cap2_LitReview/Track_Quality_Accel/Hao (2023)/Hao_results.png}
    \caption{Reconstruction results comparing the proposed model (red) with two simpler models (blue and green) and the original measurement (black). $(\boldsymbol{a})$ and $(\boldsymbol{b})$ show vertical and lateral, respectively, irregularities reconstruction for the NHHSR line. $(\boldsymbol{c})$ and $(\boldsymbol{d})$ show the vertical and lateral, respectively, irregularities reconstruction for the BGHSR line \cite{Hao02012023}.}
    \label{fig:Hao_Results}
\end{figure}

Although the metrics of Hao et al. \cite{Hao02012023} cannot be directly compared to those of De Rosa et al. \cite{DEROSA2019606} and Tsunashima and Yagura \cite{vibration7040049}, since the former employ different evaluation metrics and the latter do not report any quantitative metrics, the visual comparision show that the proposed approach achieved reconstructions much closer to the real measurements than the model-driven approach and a Gaussian Process Regression model, demonstrating its robustness in handling real-world noisy data. However, the model uses vehicle speed as an input, which may not be optimal. In this thesis, it is proposed instead to use velocity-corrected acceleration values as inputs.

Finally, hybrid methods combine the approaches of model-driven and data-driven methods \cite{Sansinena26032025}, but the contents of these works are outside the scope of this thesis, since the focus is on data-driven methods.
